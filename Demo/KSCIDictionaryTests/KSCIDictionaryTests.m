...
- (void)assertDictionary:(KSCIDictionary *)dictionary object:(id)object forKey:(NSString *)key
{
    ...
    for (NSString *casedKey in casedKeysFromKey(key)) {
        ...
        if (object) {
            STAssertTrue([[dictionary objectForKey:casedKey] isEqual:object], @"Key not found, expected object");
        } else {
            STAssertTrue(![dictionary objectForKey:casedKey], @"Key found, expected nil");
        }
    }
}
...
- (void)testCreation
{
    ...
    STAssertTrue([dictionary count] == 0, @"Dictionary is not empty");
    ...
    STAssertTrue([dictionary count] == 1, @"Dictionary count is not 1");
...
- (void)testSetting
{
    ...
    STAssertTrue([objects isEqualToArray:expectedObjects], @"Keys not found");
}
...
- (void)testFileAndURL
{
    NSString *file = [[NSBundle mainBundle] pathForResource:@"dictionary" ofType:@"plist"];
    NSLog(@"file = %@", file);
...
- (void)testCopyAndCoding
{
    ...
    STAssertTrue([dictionary isEqual:copied], @"Copied dictionary is different");
    STAssertTrue([dictionary isEqualToCIDictionary:copied], @"Copied dictionary is different");
    STAssertTrue([dictionary isEqual:serialized], @"Serialized dictionary is different");
    STAssertTrue([dictionary isEqualToCIDictionary:serialized], @"Serialized dictionary is different");

    KSCIDictionary *different = [KSCIDictionary dictionaryWithObject:@"test" forKey:@"one"];
    STAssertTrue(![dictionary isEqual:different], @"Different dictionary is equal");
    STAssertTrue(![dictionary isEqualToCIDictionary:different], @"Different dictionary is equal");
    ...
    STAssertTrue(![dictionary isEqual:lowercase], @"Mutable lowercase dictionary is equal");
    STAssertTrue(![dictionary isEqualToCIDictionary:lowercase], @"Mutable lowercase dictionary is equal");

    // immutable
    KSCIDictionary *immutableLowercase = [[lowercase copy] autorelease];
    NSLog(@"[immutableLowercase class] = %@", [immutableLowercase class]);
    STAssertTrue([dictionary isEqual:immutableLowercase], @"Immutable lowercase dictionary is different");
    STAssertTrue([dictionary isEqualToCIDictionary:immutableLowercase], @"Immutable lowercase dictionary is different");
}
